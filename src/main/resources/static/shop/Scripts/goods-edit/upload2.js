...
//选择是否随机名字
function check_object_radio2() {
    var tt = document.getElementsByName('myradio2');
...
function calculate_object_name2(filename)
{
    ...
    {
        suffix = get_suffix2(filename)
        g_object_name = g_dirname + random_string2(10) + suffix
    }
    return ''
}

function get_uploaded_object_name2(filename)
{
    if (g_object_name_type == 'local_name')
    {
...
function set_upload_param2(up, filename, ret)
{
    ...
    if (filename != '') {
        suffix = get_suffix2(filename)
        calculate_object_name2(filename)
    }
