...
accessid= 'LTAIE2MCpZVOoIva';
accesskey= 'nFBy6hjqCJNn0AficdzbaQ3nhP6zK8';
host = 'http://ossweixin.oss-cn-shanghai.aliyuncs.com/';
...
//选择是否随机名字
function check_object_radio3() {
    var tt = document.getElementsByName('myradio3');
...
function calculate_object_name3(filename)
{
    ...
    {
        suffix = get_suffix3(filename)
        g_object_name = g_dirname + random_string3(10) + suffix
    }
    return ''
}

function get_uploaded_object_name3(filename)
{
    if (g_object_name_type == 'local_name')
    {
...
function set_upload_param3(up, filename, ret)
{
    ...
    if (filename != '') {
        suffix = get_suffix3(filename)
        calculate_object_name3(filename)
    }
...
var uploader3 = new plupload.Uploader({
	...
	init: {
		PostInit: function() {
            ...
            set_upload_param3(uploader3, '', false);
            return false;
			};
		},
		...
		BeforeUpload: function(up, file) {
            check_object_radio3();
            get_dirname3();
            set_upload_param3(up, file.name, true);
        },
