							...
							if (res.rows) {
								...
								'</table>';
									...
									elemOrderData +='<tr>'+
										...
										'<td>'+(res.rows[i].orderStatus==0?'下单成功，待发货':res.rows[i].orderStatus==1?'已发货，待收货':res.rows[i].orderStatus==2?'用户退货':res.rows[i].orderStatus==3?'用户已确认收货':res.rows[i].orderStatus==4?'已由系统确认收货':'用户取消订单')+'</td>'+
